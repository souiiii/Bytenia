  ...
  const filteredServers = serversData
    .filter((server) => {
      if (activeLoc !== "all" && server.city.toLowerCase() !== activeLoc)
        return false;
      if (cpuFilter !== "Show All" && server.cpuModel !== cpuFilter)
        return false;
      if (ramFilter !== "Show All" && server.ram !== ramFilter) return false;
      if (storageFilter !== "Show All" && server.storage !== storageFilter)
        return false;
      return true;
    })
    .sort((a, b) => {
      ...
      <div className="container" style={{ maxWidth: "1200px" }}>
        ...
        <div className="ent-server-cards">
          {filteredServers.length === 0 ? (
            ...
            (isExpanded ? filteredServers : filteredServers.slice(0, 5)).map(
              (server) => (
